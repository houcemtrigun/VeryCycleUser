...
public class SendRequestAct extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        adapter = new AdapterAcceptBooking(SendRequestAct.this, arrayList);
        binding.rvAccept.setAdapter(adapter);

        binding.tvTitle.setText(getString(R.string.send_request));

        binding.ivBack.setOnClickListener(v -> {finish();});
    }

    private void getSendRrquest() {
        DataManager.getInstance().showProgressMessage(SendRequestAct.this,getString(R.string.please_wait));
        Map<String,String> map = new HashMap<>();
        map.put("user_id",DataManager.getInstance().getUserData(SendRequestAct.this).result.id);
        Log.e(TAG,"get Accept Request "+map);
        Call<RequestModel> loginCall = apiInterface.sendReq(map);
    ...
    protected void onResume() {
        super.onResume();
        if(NetworkReceiver.isConnected())  getSendRrquest();
        else App.showToast(SendRequestAct.this,getString(R.string.network_failure), Toast.LENGTH_LONG);
    }
}
